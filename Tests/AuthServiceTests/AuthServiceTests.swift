...
class MockedSuccessNetworkService: NetworkProvider {
    ...
    public var tokenObj: Decodable? = Token(token: "token")
    ...
    func performRequest<T>(for resource: Resource, decodingTo type: T.Type, completion: @escaping (Result<T, NetworkError>) -> Void) where T: Decodable {
        ...
        guard let tokenObj = tokenObj as? T else {
            completion(.failure(.badData))
            return
        }

        completion(.success(tokenObj))
    }
...
final class AuthServiceTests: XCTestCase {
    ...
    func testLogin() {
        ...
        var authService = AuthService(networkService: MockedSuccessNetworkService(),
        ...
        authService = AuthService(networkService: failNetworkService,
                                  secureStorage: MockedSuccessSecureStorage(),
                                  config: config)
        ...
        authService = AuthService(networkService: MockedSuccessNetworkService(),
        ...
        // Secure Storage - OK
        authService = AuthService(networkService: MockedFailNetworkService(),
                                  secureStorage: MockedSuccessSecureStorage(),
                                  config: config)
    ...
    func testLogoutSuccess() {
        ...
        do {
            ...
            let authService = AuthService(networkService: MockedSuccessNetworkService(),
